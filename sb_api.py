...
def _load_config(config_path: str = "config/sb_config.yaml") -> Dict:
    ...
    Required config:
      ...
      stonebranch.username  (or env: SB_USERNAME)
      stonebranch.password  (or env: SB_PASSWORD)
    """
    cfg = {}
    ...
    cfg["username"]    = os.environ.get("SB_USERNAME",  cfg.get("username", ""))
    cfg["password"]    = os.environ.get("SB_PASSWORD",  cfg.get("password", ""))
    ...
    if not cfg["username"] or not cfg["password"]:
        raise ValueError(
            "Stonebranch credentials not set. "
            "Set SB_USERNAME and SB_PASSWORD environment variables, "
            "or configure config/sb_config.yaml."
        )
